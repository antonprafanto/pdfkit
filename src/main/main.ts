...
function createWindow(): void {
  ...
  mainWindow.once('ready-to-show', () => {
    ...
    // If launched with PDF files, send them to renderer after a short delay
    // to ensure React is fully mounted
    if (filesToOpen.length > 0 && mainWindow) {
      setTimeout(() => {
        filesToOpen.forEach((filePath) => {
          mainWindow?.webContents.send('open-pdf-file', filePath);
        });
        filesToOpen = []; // Clear after sending
      }, 500);
    }
  });
  ...
  if (mainWindow) {
    ...
    if (!isDev) {
      ...
      ipcMain.handle('updater:check', async () => ({
        checking: false,
        available: false,
        error: 'Dev mode - updater disabled',
      }));
...
ipcMain.handle(
  'print-pdf',
  async (_event, options?: { pdfPath?: string; pdfBytes?: Uint8Array; fileName?: string }) => {
    try {
...
ipcMain.handle(
  'convert-pdf-with-libreoffice',
  async (_, pdfBytes: Uint8Array, format: 'docx' | 'xlsx') => {
    const buffer = Buffer.from(pdfBytes);
    ...
    try {
      ...
      if (images.length === 0) {
        return {
          ...
          count: 0,
          message:
            'No traditional images found. For PDFs from Word/print, use "Export as Images" (Ekspor sebagai Gambar) instead.',
        };
      }
...
ipcMain.handle(
  'web-optimize-pdf',
  async (_, pdfBytes: Uint8Array, options: { quality: string }) => {
    try {
      const { execSync } = require('child_process');
      const os = require('os');
      ...
      try {
        ...
        return {
          success: false,
          error: 'Failed to optimize PDF: ' + (gsError.message || 'Ghostscript error'),
        };
      }
    } catch (error: any) {
