 ...
 * Dialog for extracting specific pages to a new PDF document
 */
...
export function ExtractPagesDialog({ open, onClose }: ExtractPagesDialogProps) {
  ...
  const handleExtract = async () => {
    ...
    try {
      ...
      const extractedPdfBytes = await pdfManipulationService.extractPages(file, pagesToExtract);

      setModifiedPdf(extractedPdfBytes);
      setProcessing(false, 100);

      // Save the extracted PDF
      const defaultName = fileName?.replace('.pdf', '_extracted.pdf') || 'extracted_pages.pdf';
      const filePath = await window.electronAPI.saveFileDialog(defaultName);

      if (filePath) {
        const result = await window.electronAPI.savePdfFile(filePath, extractedPdfBytes);
      ...
      footer={
        <div className="flex justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button variant="outline" onClick={clearSelection} disabled={isExtracting}>
              Clear
            </Button>
            <Button onClick={handleExtract} disabled={selectedPages.size === 0 || isExtracting}>
              {isExtracting ? <Spinner size="sm" /> : `Extract ${selectedPages.size} page${selectedPages.size !== 1 ? 's' : ''}`}
            </Button>
          </div>
        </div>
      }
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-md bg-blue-50 p-3 dark:bg-blue-900/20">
          <p className="text-sm text-blue-700 dark:text-blue-300">
            Document has {totalPages} pages • {selectedPages.size} selected for extraction
          </p>
        </div>
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((pageNum) => {
              ...
              return (
                <button
                  ...
                  `}
                  title={isSelected ? `Remove page ${pageNum} from selection` : `Select page ${pageNum} for extraction`}
                >
                  {/* Page icon */}
                  <svg
                    className={`h-8 w-8 ${isSelected ? 'text-green-600 dark:text-green-400' : 'text-gray-400'}`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                  ...
                  {/* Page number */}
                  <span className={`mt-1 text-xs font-medium ${isSelected ? 'text-green-700 dark:text-green-300' : 'text-gray-700 dark:text-gray-300'}`}>
                    {pageNum}
                  </span>
        ...
        <div className="rounded-md bg-gray-50 p-3 dark:bg-gray-800">
          <p className="text-xs text-gray-600 dark:text-gray-400">
            <strong>Note:</strong> The original document will remain unchanged. Selected pages will be copied to a new PDF file.
          </p>
        </div>
      </div>
    </Dialog>
